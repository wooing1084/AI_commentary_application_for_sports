...
class FeetDetector:
    ...
    def get_players_pos(self, M, M1, frame, timestamp, map_2d, data):
        warped_kpts = []
        ...
        indexes_ppl = np.array([np.array(np.where(p == True)).T for p in ppl])
        ...
        for kpt in warped_kpts:
            ...
            if len(iou_scores) > 0:
                ...
                #                   left x,top y , w, h 
                max_iou[1].bboxs[timestamp] = (bbox[1] + PAD, bbox[0] + PAD, (bbox[3] - bbox[1] - 2 * PAD) ,( bbox[2] - bbox[0] - 2 * PAD))

                # else:
                # max_iou[1].positions[timestamp] = (homo[0], homo[1], 행동인식)
                cv2.rectangle(frame, (bbox[1] + PAD, bbox[0] + PAD), (bbox[3] - PAD, bbox[2] - PAD), (0, 255, 0), 2)
            else:
                # first detection
                for player in self.players:
                    if (player.team == color_key) and (player.previous_bb is None):
                        ...
                        player.bboxs[timestamp] = (bbox[1] + PAD, bbox[0] + PAD, (bbox[3] - bbox[1] - 2 * PAD) ,( bbox[2] - bbox[0] - 2 * PAD))
                        # player.positions[timestamp] = (homo[0], homo[1], 행동인식)
                        break
...
#                    player.bboxs = {}
        ...
        for p in self.players:
            if p.team != 'referee':
                print(p)
                try:
                    # cv2.rectangle(frame, (p.bboxs[0], p.bboxs[1]), (p.bboxs[2], p.bboxs[3]), (0, 255, 0), 2)
                    # create object for print
                    # cv2.rectangle(frame,
                    #               (p.bboxs[timestamp][0], p.bboxs[timestamp][1]),
                    #               (p.bboxs[timestamp][2] + p.bboxs[timestamp][0], p.bboxs[timestamp][3] + p.bboxs[timestamp][1]),
                    #               (0, 255, 0), 2)
                    ...
                    # axis_text = 'x1: ' + str(p.bboxs[timestamp][0]) + ' y1: ' + str(p.bboxs[timestamp][1]) + ' x2: ' + str(p.bboxs[timestamp][2] + p.bboxs[timestamp][0]) + ' y2: ' + str( p.bboxs[timestamp][3] + p.bboxs[timestamp][1])
                    # axis_text_size, _ = cv2.getTextSize(str(axis_text), cv2.FONT_HERSHEY_SIMPLEX, 0.7, 1)
                    # axis_text_origin = (p.bboxs[timestamp][0] - axis_text_size[0] // 2,
                    #                     p.bboxs[timestamp][1] + axis_text_size[1] // 2)
                    # cv2.putText(frame, str(axis_text), axis_text_origin,
                    #             cv2.FONT_HERSHEY_SIMPLEX, 0.4,
                    #             (0, 255, 0), 1, cv2.LINE_AA)
                    cv2.putText(map_2d_text, str(p.ID), text_origin,
                                cv2.FONT_HERSHEY_SIMPLEX, 1.5,
                                (0, 0, 0), 3, cv2.LINE_AA)
                    # data.append({"FRAME": timestamp, "TEAM": p.team, "X1": p.bboxs[timestamp][0], "Y1": p.bboxs[timestamp][1],
                    #              "X2": p.bboxs[timestamp][2] + p.bboxs[timestamp][0], "Y2": p.bboxs[timestamp][3] + p.bboxs[timestamp][1]})
                except KeyError:
                    pass
