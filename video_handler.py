...
class VideoHandler:
    ...
    def run_detectors(self):
            ...
            if not ok:
                ...
                if 0 <= time_index <= 230:
                    ...
                    for player in self.feet_detector.players:
                        player.bboxs[time_index] = (0, 0, 0, 0)
                    ...
                    #vis = cv2.imread(cv2.resize(map_2d_text, (frame.shape[1], frame.shape[1] // 2)))
                    cv2.imshow("Tracking", vis)
                    # plt_plot(vis)
                    writer.writeFrame(cv2.cvtColor(cv2.resize(vis, (frame.shape[1], frame.shape[1] // 2)), cv2.COLOR_BGR2RGB))
                    self.frames.append(frame)
                    
                    image_path = os.path.join("outputs", f"result_{time_index}.png")
        ...
        #     writer = csv.DictWriter(file, fieldnames=["FRAME", "TEAM", "X1", "Y1", "X2", "Y2"])
